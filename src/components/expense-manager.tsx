...
const CATEGORY_STYLES: Record<ExpenseCategory, string> = {
  SALARY: "bg-sky-50 text-sky-700 ring-1 ring-inset ring-sky-200",
  RENT: "bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-200",
  UTILITIES: "bg-violet-50 text-violet-700 ring-1 ring-inset ring-violet-200",
  MARKETING: "bg-pink-50 text-pink-700 ring-1 ring-inset ring-pink-200",
  SUPPLIES: "bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-200",
  OTHER: "bg-slate-100 text-slate-500 ring-1 ring-inset ring-slate-200",
};
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleCreate} className="rounded-xl border border-sky-100 bg-white p-5 shadow-sm">
        <h2 className="mb-3 text-sm font-semibold text-slate-800">Add an expense</h2>
        {error && (
      ...
      <div className="overflow-x-auto rounded-xl border border-sky-100 bg-white shadow-sm">
        <table className="w-full text-left text-sm">
          ...
          {expenses.length > 0 && (
            <tfoot>
              <tr className="border-t border-slate-200 bg-sky-50/40 font-medium text-slate-700">
...
function ExpenseRow({
  ...
  return (
    <tr className={`border-t border-slate-100 transition-colors hover:bg-sky-50/40 ${expense.voided ? "opacity-50" : ""}`}>
      <td className="px-3 py-2">
      ...
      <td className="px-3 py-2 text-slate-700">{expense.branch?.name ?? "Org-wide"}</td>
      <td className="px-3 py-2 text-slate-700">{expense.employee?.name ?? "—"}</td>
      <td className="px-3 py-2 whitespace-nowrap text-slate-700">{formatDate(expense.expenseDate)}</td>
      <td className="px-3 py-2 whitespace-nowrap text-slate-700">{formatMoney(expense.amount)}</td>
      <td className="px-3 py-2">

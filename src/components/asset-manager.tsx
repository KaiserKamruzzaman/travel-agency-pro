...
const STATUS_STYLES: Record<AssetStatus, string> = {
  IN_USE: "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 ring-1 ring-inset ring-emerald-200 dark:ring-emerald-800",
  SPARE: "bg-sky-50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-300 ring-1 ring-inset ring-sky-200 dark:ring-sky-800",
  UNDER_REPAIR: "bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-300 ring-1 ring-inset ring-amber-200 dark:ring-amber-800",
  RETIRED: "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 ring-1 ring-inset ring-slate-200 dark:ring-slate-700",
};
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleCreate} className="rounded-xl border border-sky-100 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-sm">
        <h2 className="mb-3 text-sm font-semibold text-slate-800 dark:text-slate-200">Add an asset</h2>
        {error && (
        ...
        {branches.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">Add a branch first — assets must be assigned to one.</p>
        ) : (
          <>
  ...
  if (editing) {
    return (
      <tr className="border-t border-slate-100 dark:border-slate-800 align-top">
        ...
        <td className="px-3 py-2 text-slate-500 dark:text-slate-400">{STATUS_LABEL[asset.status]}</td>
        <td className="px-3 py-2">
          <input
  ...
  return (
    <tr className="border-t border-slate-100 dark:border-slate-800 transition-colors hover:bg-sky-50/40 dark:hover:bg-sky-950/20">
      <td className="px-3 py-2">
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400">{asset.category}</div>
      </td>
      <td className="px-3 py-2 text-slate-700 dark:text-slate-300">{asset.branch.name}</td>
      <td className="px-3 py-2 text-slate-700 dark:text-slate-300">{asset.serialNumber || `Qty: ${asset.quantity}`}</td>
      <td className="px-3 py-2 text-slate-700 dark:text-slate-300">{asset.assignedTo?.name ?? "—"}</td>
      <td className="px-3 py-2 text-slate-700 dark:text-slate-300">
        {asset.purchaseDate ? formatDate(asset.purchaseDate) : "—"}
        {asset.purchasePrice && <div className="text-xs tabular-nums text-slate-500 dark:text-slate-400">{formatMoney(asset.purchasePrice)}</div>}
      </td>
      <td className="px-3 py-2">
      ...
      <td className="px-3 py-2">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            className="rounded-lg border border-slate-200 dark:border-slate-700 px-2 py-1 text-xs text-slate-700 dark:text-slate-300 outline-none focus:border-sky-400 dark:focus:border-sky-500"
            value={asset.status}
            disabled={pending}
